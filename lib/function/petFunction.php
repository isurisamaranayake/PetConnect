...
class pet extends Main{
    ...
    public function addpet($type, $breed, $name, $gender, $pet_age, $pet_size, $pet_color,$pet_location, $otherdetails, $imageName, $imageType, $imageLocation){
    ...
    public function filteredpetlistforadoption($category,$age,$size,$location){
        ...
        $getquery = "SELECT * FROM pet_tbl WHERE d_status = 0 AND status = 'approved' AND ('$category' = '--' OR type = '$category') 
        
        AND ('$age' = '--' OR pet_age = '$age') AND ('$size' = '--' pet_size = '$size') AND ('$location' = '--' OR petlocation = '$location') AND cBy != ' $cBy';";
